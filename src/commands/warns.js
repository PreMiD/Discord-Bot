...
exports.run = async (client, message, params) => {
	...
	if (params.length < 1) {
		...
		if (warns == 0) {
			message
				.reply("You don't have any warnings, good boi <:PUDDING:520986516613234711>")
			...
			message.reply("I DM'ed you your warnings!").then((msg) => setTimeout(() => msg.delete(), 10 * 1000));
		}
	} else {
		if (client.elevation(message) > 0) {
			var user = message.mentions.users.first();
			if (!user)
				user = await message.guild.members.find(
					(m) => m.displayName == params.slice(0, params.length).join(' ')
				);
			if (user) {
				var warns = (await query('SELECT * FROM warns WHERE user_id = ?', user.id)).rows;
				if (warns == 0) {
					message
						.reply(`${message.guild.members.get(user.id).displayName} has no warnings.`)
						.then((msg) => setTimeout(() => msg.delete(), 10 * 1000));
				} else {
					var embed = new Discord.RichEmbed()
			...
			message
				.reply("Only Moderators can view other people's warnings!")
				.then((msg) => setTimeout(() => msg.delete(), 10 * 1000));
		}
	}
};
