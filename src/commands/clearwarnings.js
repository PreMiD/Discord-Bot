...
exports.run = async (client, message, params) => {
	...
	if (params.length < 1) {
		message.reply(`Usage: \`\`${prefix}clearwarn <user>\`\``).then((msg) => setTimeout(() => msg.delete(), 10 * 1000));
	} else {
		...
		if (user) {
			...
			if (!user.bot) {
				...
				message.reply("Bots don't have warnings, or do they :thinking:").then((msg) => setTimeout(() => msg.delete(), 10 * 1000));
			}
		}
	}
};
...
exports.conf = {
	enabled: true,
	aliases: [ 'cwarn', "cwarnings", "clearwarnings" ],
	permLevel: 3
};
