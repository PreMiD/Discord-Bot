...
async function updateCredits() {
	...
	Promise.all(results).then((completed) => {
		...
		completed.map((result) => {
			...
			if (result[0].roles.has(pat20Role)) patronColor = result[0].roles.get(pat20Role).hexColor;
			if (result[0].roles.has(pat10Role)) patronColor = result[0].roles.get(pat10Role).hexColor;
			if (result[0].roles.has(pat5Role)) patronColor = result[0].roles.get(pat5Role).hexColor;
			if (result[0].roles.has(pat1Role)) patronColor = result[0].roles.get(pat1Role).hexColor;
