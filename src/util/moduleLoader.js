...
var moduleFolder = path.resolve("./modules");
...
async function loadCommands(filePath, client) {
    ...
    files.map(command => {
        ...
        if (typeof props["config"] == "undefined") {
            error(`Command ${command} in module ${path.basename(path.dirname(filePath))} is missing required field config`);
            return;
        }
        if (typeof props.config.name == "undefined") {
            error(`Command ${command} in module ${path.basename(path.dirname(filePath))} is missing required property name`);
            return;
        }
...
async function loadEvents(filePath, client) {
    var eventFile, files = await fs.readdirSync(filePath);
    ...
    files.map(event => {
        ...
        if (typeof eventFile == "function")
            ...
            if (typeof eventFile.config == "undefined") {
                error(`Event ${event} in module ${path.basename(path.dirname(filePath))} is missing required field config`);
                return;
            }
