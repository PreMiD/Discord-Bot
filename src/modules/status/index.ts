        ...
        if (body.incidents.length > 0) {
            ...
            //* Bot doesn't know about this incident yet
                ...
                cache.lastIncident = {
                    ...
                    components: incident.components.map((c) => { return c.name; }),
                    url: incident.shortlink,
                    createdAt: new Date(incident.created_at)
                };

                cache.lastUpdate = {
                    id: incident.incident_updates[incident.incident_updates.length - 1].incident_id,
                    body: incident.incident_updates[incident.incident_updates.length - 1].body,
                    status: incident.incident_updates[incident.incident_updates.length - 1].status,
                    createdAt: new Date(incident.incident_updates[incident.incident_updates.length - 1].display_at)
                };
