...
var { prefix } = require("../../../config.json"), embed = new Discord.MessageEmbed({
    title: "Warn",
...
module.exports.run = async (message, params) => {
    ...
    var warns = 1 +
        //@ts-ignore
        (await db_1.db.query("SELECT COUNT(user_id) AS warns FROM warns WHERE user_id = ?", 
        //@ts-ignore
        message.mentions.users.first().id))[0][0].warns;
    db_1.db.query("INSERT INTO warns (user_id, reason, moderator_id) VALUES (?, ?, ?)", [
        //@ts-ignore
