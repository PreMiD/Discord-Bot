...
module.exports.run = async (message: Discord.Message, args: Array<string>) => {
    ...
    const m = await message.reply(null, new Discord.MessageEmbed({ title: 'Presence Search Query', description: '<a:loading:521018476480167937> Searching...' }));

    const mention = message.member.toString() + ', ';

    const sadCatTimeout = setTimeout(async () => {
        await m.edit(mention, new Discord.MessageEmbed({ title: 'Presence Search Query', description: '<a:loading:521018476480167937> Searching...\n\n<:sad_cat:587782591847989288> Looks like you drew the short straw! We are still indexing the database, so your query will take a while.' }));
    }, 10000);

    const query = args.join(" ");
    const presencesColl = pmdDB.collection("presences");
    const results = await presencesColl.find({
        $text: {
            $search: query,
            $caseSensitive: false
        }
    ...
    for (const result of results) {
        descriptionFunsies += `**[${result.name}](https://premid.app/store/presences/${encodeURIComponent(result.name)} "Click here to go to the store page for ${result.name}!")** by [${result.metadata.author.name}](https://premid.app/users/${result.metadata.author.id} "Click here to go to the profile page for ${result.metadata.author.name}!")`;
        descriptionFunsies += '\n_';
    ...
    return await m
        .edit(mention, new Discord.MessageEmbed({
